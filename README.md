...
$h_j$ is representing the distribution of the monosemantic features across the $j$-th example. For example, if $j$ is `gtcact`, then $h_j$ would be close to a one-hot encoding of the DNA feature, and if $j$ is `مرحبا`, then $h_j$ would be close to a one-hot encoding of the Arabic feature.
...
where $f_i(x_j)$ is the activation of the $i$-th monosemantic feature on the $j$-th parameter of the MLP output $x^j$. 
